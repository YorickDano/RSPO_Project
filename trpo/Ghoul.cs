...
namespace trpo
{
    public partial class Ghoul : Form
    { //Данный метод отвечает за создание окна
        ...
        //Данный метод отвечает за отображение графического интерфейса при закгузке окна
        private void Ghoul_Load(object sender, EventArgs e)
        {
            DataBase bd = new DataBase();
            MySqlCommand commandOut = new MySqlCommand("SELECT * FROM `1000-7=?`", bd.getConn());
        ...
        private void Ghoul_MouseDown(object sender, MouseEventArgs e)
        {
            lp = new Point(e.X, e.Y);
        }
        //Данный метод отвечает за перемещение окна
        private void Ghoul_MouseMove(object sender, MouseEventArgs e)
        {
